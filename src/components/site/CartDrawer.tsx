...
import { Check, MapPin, Minus, Plus, ShoppingBag, Trash2, X } from "lucide-react";
...
const CartDrawer = () => {
  ...
  const quote = useMemo(
    () => quoteDelivery(postcode, subtotalAfterDiscount),
    [postcode, subtotalAfterDiscount]
  );

  const deliveryFee = method === "deliver" && quote.zone ? quote.fee : 0;
  const canDeliver = method !== "deliver" || quote.ok;
  const total = Math.max(0, subtotalAfterDiscount + deliveryFee);
        ...
        <div className="flex h-full flex-col">
          ...
          {step === "cart" ? (
            <>
              ...
              {lines.length > 0 && (
                <div className="border-t border-gold/15 p-5 space-y-4 max-h-[60vh] overflow-y-auto">
                  <div className="flex gap-2 rounded-full gold-border p-1">
                    {(["deliver", "pickup"] as Method[]).map((mm) => (
                      <button
                        key={mm}
                      ...
                      >
                        {mm === "deliver" ? t("cart.deliver") : t("cart.pickup")}
                      </button>
                    ))}
                  </div>
                  ...
                  {method === "deliver" && (
                    <div className="space-y-2">
                      <label className="block">
                        <span className="text-xs uppercase tracking-widest text-muted-foreground">{t("cart.postcodeLabel")}</span>
                        <div className="mt-1.5 flex items-center gap-2 rounded-full gold-border bg-ink px-4 py-2 focus-within:border-gold/60">
                          ...
                          <input
                            ...
                            placeholder="1211"
                            className="flex-1 bg-transparent text-sm text-ivory placeholder:text-muted-foreground focus:outline-none"
                          />
                        </div>
                      </label>
                      {postcode.length === 4 && (
                  ...
                  {method === "deliver" && quote.reason === "below-minimum" && quote.zone && (
                    <div className="text-xs text-accent">
                      {t("cart.minOrder")} €{quote.zone.minOrder.toFixed(2)} ({quote.zone.area})
                    </div>
                  )}
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <label className="block">
                      ...
                      <input required defaultValue={quote.zone?.area ?? "Hilversum"} className="mt-1.5 w-full rounded-xl gold-border bg-ink px-4 py-2.5 text-sm text-ivory focus:outline-none focus:border-gold/60" />
                    </label>
                  </div>
                </>
              )}
